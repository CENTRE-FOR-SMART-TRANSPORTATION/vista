...
def multiprocessed_count_funct(input_tuple: tuple):
    ...
    numFrame = get_first_number(filename)
    pc = np.divide(file,1000) # convert from mm to meters
    inputExtended = (pc,voxel_rsize, voxel_asize, voxel_esize, data, i, total_voxels)
...
def occupancy_count(input: tuple) -> np.float32:
    ...
    if not USE_CARTESIAN:
        ...
        out_ratio = vx.shape[0] / total_voxels # Pass total voxels from outside of the loop
...
def data_rate_vista_automated(
    ...
    ## temporary
    volume_far = (1/3)*(math.pow(245,3) - math.pow((245-0.03),3))\
        *(np.cos(np.deg2rad(0))-np.cos(np.deg2rad(0.11)))\
            *(np.deg2rad(0.11)-np.deg2rad(0))
    volume_near = (1/3)*(math.pow(0.03,3) - math.pow(0,3))\
        *(np.cos(np.deg2rad(0))-np.cos(np.deg2rad(0.11)))\
            *(np.deg2rad(0.11)-np.deg2rad(0))
    ## temporary
    ...
    print(f"volume far:\n{volume_far}")
    print(f"volume near:\n{volume_near}")
    print(f"average:\n{(volume_far+volume_near)/2}")
    
    # Test
    print('vista path', vistaoutput_path)
    ...
    for itr in range(numScenes):
        ...
        if USE_VOLUMETRIC:
            with mp.Pool(numCores) as p:
                inputData = [(voxel_rsize, voxel_asize, voxel_esize, data,i\
                            ,vistaoutput_path[itr], point_density, max_volume, global_offset) for i in range(smallest, upperbound,resolution)]
                results = []
        ...
        with mp.Pool(numCores) as p:
            inputData = [(voxel_rsize, voxel_asize, voxel_esize, data, i\
                        ,vistaoutput_path[itr], point_density, total_voxels, global_offset) for i in range(smallest, upperbound, resolution)]
            results = []
    ...
    if USE_VOLUMETRIC:
        ...
        for i in range(numScenes):
            outmatrix_volume2[i][:, 1] = outmatrix_volume2[i][:, 1] / np.max(outmatrix_volume2[i][:, 1])  # BANDAID FIX
    ...
    for scene_number in range(numScenes):
        output_folder = os.path.join("data_rates", os.path.basename(vistaoutput_path[scene_number]))
        ...
        np.savetxt(os.path.join(output_folder, "an_data_rate_count.csv"), 
                np.concatenate((outmatrix_count[scene_number][:,0][:, np.newaxis], an_data_rate2[scene_number]), axis=1), 
                delimiter=',')
        ...
        if USE_VOLUMETRIC:
            ...
            np.savetxt(os.path.join(output_folder, "an_data_rate_volume.csv"), 
                    np.concatenate((outmatrix_volume2[scene_number][:,0][:, np.newaxis], an_data_rate[scene_number]), axis=1), 
                    delimiter=',')
    ...
    for itr in range(numScenes):
        ...
        if USE_VOLUMETRIC:
            an_data_rate_ave.append(rolling_average(an_data_rate[itr],0)) # BANDAID FIX (we do not care about the volume method for now)
        else:
            pass
    ...
    def showDataRateGraph(xBarData,yBarData,yBarAverageData,windowTitle,graphTitle,xlabel,ylabel,isSimple):
        ...
        ax1.set_ylabel(f"{ylabel} {get_folder(vistaoutput_path[0])}",color='r')
        ...
        ax2.set_ylabel(f"{ylabel} {get_folder(vistaoutput_path[0])}",color='r')
        ...
        ax3.set_ylabel(f"{ylabel} {get_folder(vistaoutput_path[0])}",color='r')
        ...
        # fig4.suptitle(f"{graphTitle} with multiple y-scale", fontsize=12) # BANDAID FIX
        ...
        for i in range(numScenes):
            if i == 0:
                ...
                #Setting new Y-axis
                # We don't need the road section name for the axis label.
